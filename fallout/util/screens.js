...
export async function boot() {
        ...
        else if(triggerParam=="unlock") {
        await power()
        say("AUTHENTICATION SUCCESS")
        outro()
        }
...
if (storedLanguage === 'pt') {
  ...
  await type('// TAPEZ AIDE POUR VOIR LES COMMANDES OU RETOUR POUR REVENIR');
	...
	else if (storedLanguage === 'ja') {  // Japanese
  await type('>/ ロブコインダストリーズ (TM) ターミリンクプロトコル');
  await type('// コマンドを見るにはHELPをタイプするか、戻るにはRETURNをタイプしてください');
} 
else if (storedLanguage === 'ru') {  // Russian
  await type('>/ ROBCO INDUSTRIES (TM) ТЕРМЛИНК ПРОТОКОЛ');
  await type('// ВВЕДИТЕ HELP ДЛЯ ПОКАЗА КОМАНД ИЛИ ВЕРНУТЬСЯ ВВЕДИТЕ RETURN');
}
else {
  await type('>/ ROBCO INDUSTRIES (TM) TERMLINK PROTOCOL');
  await type('// TYPE HELP TO SEE COMMANDS OR RETURN TO GO BACK');
...
if (command == "help") {
  ...
  type("> ROBCO INDUSTRIES 2075-2077\nЗАДАЙТЕ ВОПРОС ИЛИ ЗАПРОС\nНАПИШИТЕ 'CLS' ДЛЯ ОЧИСТКИ ЭКРАНА\nНАПИШИТЕ 'НАЗАД' ДЛЯ ВОЗВРАТА\n\nДОСТУПНЫЕ КОМАНДЫ:\n\n- clear         - cowsay\n- debug         - dino\n- fallout       - fire\n- friend        - hack\n- home          - logout\n- matrix        \n- pokemon       - quit\n- reboot        - rick\n- rogue         - screensaver\n- systemshock   - text-editor");
}
