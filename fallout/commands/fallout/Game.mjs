...
Game.prototype.updateLives = async function() {
	...
	let blocks = Array(this.lives)
		...
		.map(() => "■ ")
		.join("");
		 saud("Attempts remaininng A B C D")
...
Game.prototype.start = async function() {
    saud("Welcome to ROBCO Industries (TM) Termlink Interfaceand  Password Required")
    await type(
        ["Welcome to ROBCO Industries (TM) Termlink Interface", "/F:== Check browser interface console logs", "Password Required"],
        FAST,
        this.container
    );
    // Get list of words
